...
#sonic
CUDA_VISIBLE_DEVICES=1 python CEEC/CEEC-hirise-infer.py --dataset mars_hirise --cluster "0"
CUDA_VISIBLE_DEVICES=2 python CEEC/CEEC-hirise-infer.py --dataset mars_hirise --cluster "1"
CUDA_VISIBLE_DEVICES=7 python CEEC/CEEC-hirise-infer.py --dataset mars_hirise --cluster "2"
CUDA_VISIBLE_DEVICES=1 python CEEC/CEEC-hirise-infer.py --dataset mars_hirise --cluster "3"
CUDA_VISIBLE_DEVICES=6 python CEEC/CEEC-hirise-infer.py --dataset mars_hirise --cluster "4"
CUDA_VISIBLE_DEVICES=4 python CEEC/CEEC-hirise-infer.py --dataset mars_hirise --cluster "all"
...
os.makedirs("CEEC_only_infer_results/{}{}_images".format(prefix, opt.dataset), exist_ok=True)
...
if opt.l1_adv:
    ...
    PATH = "CEEC_only_results/{}{}_discriminator.h5f".format(prefix, opt.dataset)
print('Loading from ', PATH)
ckpt = torch.load(PATH, map_location='cpu')
...
#         import ipdb; ipdb.set_trace()
    ...
    save_image(sample, "CEEC_only_infer_results/{}{}_images/%d.png".format(prefix, opt.dataset) % i, nrow=8, normalize=True)
    
    # Save sample
    step=i

    save_each_image((imgs.data + 1)/2, "CEEC_only_infer_results/{}{}_images".format(prefix, opt.dataset), 'clean', step=step)
    save_each_image((masked_imgs_display.data + 1)/2, "CEEC_only_infer_results/{}{}_images".format(prefix, opt.dataset), 'noisy', step=step)
    save_each_image((outputs_merged.data + 1)/2, "CEEC_only_infer_results/{}{}_images".format(prefix, opt.dataset), 'reconstructed', step=step)
