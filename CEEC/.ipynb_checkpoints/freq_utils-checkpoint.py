...
def get_hirise_masked(x_im, mask_split='train'): # b x 1 x h x w 
    N = len(x_im)
    assert len(x_im[0].shape)==3, "Image should be color" #pytorch 1 x h x w
    h, w = x_im[0].shape[1:]
...
#     sample_mask = train_masks if train else test_masks
